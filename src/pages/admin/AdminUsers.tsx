...
import { Search, Loader2, Ban, CheckCircle, Trash2, Edit, Shield, KeyRound, Calendar } from "lucide-react";
...
export default function AdminUsers() {
  ...
  const [editData, setEditData] = useState({ full_name: "", email: "", cpf: "", phone: "", subscription_ends_at: "" });
  ...
  const handleSaveEdit = async () => {
    ...
    if (data.subscription_ends_at) data.subscription_ends_at = new Date(data.subscription_ends_at).toISOString();
    else delete data.subscription_ends_at;
    await handleAction("update_user", editUser.user_id, { data });
    setEditUser(null);
  };
  ...
  const getSubStatus = (user: UserProfile) => {
    if (!user.subscription_ends_at) {
      if (user.trial_ends_at && new Date(user.trial_ends_at) > new Date()) return { label: "Trial", variant: "secondary" as const };
      return { label: "Sem assinatura", variant: "outline" as const };
    }
    if (new Date(user.subscription_ends_at) > new Date()) return { label: "Ativo", variant: "default" as const };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar nome, e-mail ou CPF..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()} className="pl-10" />
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="text-left p-3 font-medium hidden md:table-cell">Assinatura</th>
                    <th className="text-left p-3 font-medium hidden md:table-cell">Cargo</th>
                    <th className="text-left p-3 font-medium hidden lg:table-cell">Criado</th>
                  ...
                  {users.map((user) => {
                    ...
                    return (
                      <tr key={user.user_id} className="border-b hover:bg-muted/30">
                        ...
                        <td className="p-3 hidden md:table-cell"><Badge variant={sub.variant} className="text-xs">{sub.label}</Badge></td>
                        <td className="p-3 hidden md:table-cell">
                          <div className="flex gap-1 flex-wrap">
                            {user.is_system_admin && <Badge className="text-xs bg-purple-600">Admin</Badge>}
                            {user.roles.map((r) => <Badge key={r} variant="secondary" className="text-xs">{r}</Badge>)}
                          </div>
                        </td>
                        <td className="p-3 hidden lg:table-cell text-muted-foreground">{format(new Date(user.created_at), "dd/MM/yyyy")}</td>
                        <td className="p-3">
                          <div className="flex gap-1 justify-end flex-wrap">
                            <Button size="icon" variant="ghost" onClick={() => openEdit(user)} title="Editar"><Edit className="w-4 h-4" /></Button>
                            <Button size="icon" variant="ghost" onClick={() => { setPasswordDialog(user); setNewPassword(""); }} title="Redefinir senha"><KeyRound className="w-4 h-4" /></Button>
                            {user.is_blocked ? (
                              <Button size="icon" variant="ghost" onClick={() => handleAction("unblock_user", user.user_id)} disabled={actionLoading === user.user_id} title="Desbloquear">
                                {actionLoading === user.user_id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4 text-green-500" />}
                              </Button>
                            ) : (
                              <Button size="icon" variant="ghost" onClick={() => handleAction("block_user", user.user_id)} disabled={actionLoading === user.user_id || user.is_system_admin} title="Bloquear">
                                {actionLoading === user.user_id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Ban className="w-4 h-4 text-red-500" />}
                              </Button>
                            )}
                            <Button size="icon" variant="ghost" onClick={() => setDeleteUser(user)} disabled={user.is_system_admin} title="Deletar"><Trash2 className="w-4 h-4 text-red-500" /></Button>
                          </div>
                        </td>
                      </tr>
                    );
                  })}
                  {users.length === 0 && <tr><td colSpan={8} className="text-center py-10 text-muted-foreground">Nenhum usuário encontrado.</td></tr>}
                </tbody>
              </table>
            </div>
          )}
      ...
      <Dialog open={!!editUser} onOpenChange={(open) => !open && setEditUser(null)}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div><Label>Assinatura até</Label><Input type="date" value={editData.subscription_ends_at} onChange={(e) => setEditData({ ...editData, subscription_ends_at: e.target.value })} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditUser(null)}>Cancelar</Button>
            <Button onClick={handleSaveEdit} disabled={actionLoading === editUser?.user_id}>Salvar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!passwordDialog} onOpenChange={(open) => !open && setPasswordDialog(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Redefinir Senha - {passwordDialog?.full_name}</DialogTitle></DialogHeader>
          <div className="space-y-4">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta ação é irreversível. <strong>{deleteUser?.full_name}</strong> ({deleteUser?.email}) será removido permanentemente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
